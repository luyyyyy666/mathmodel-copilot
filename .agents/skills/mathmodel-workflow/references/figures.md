...
先核对 problem.md/model.md 中的小问和图表计划，再按实际结果更新：每张图对应哪个结论、需要哪些数据、计划放在论文哪里。数据不足时补计算或改用有效的表达，不能补造曲线。可删除不能增加信息的计划图并说明理由；不要求所有探索图进入论文。

图注说明比较对象、数据范围、单位、重复次数和区间含义，并指出实际观察及限制。把置信区间、预测区间、参数扰动区间和仿真波动分开。按最终版面尺寸检查可读性；以清楚表达为目标，不强制用复杂图替代简单图。
